...
const ExpendCategoryItem = React.memo(
  ({ item, active, onPressCategoryItem }) => {
    return (
      <ExpandPanelItem
        key={item.id}
        ...
        <ExpandPanelItemText active={active}>{item.name}</ExpandPanelItemText>
        {active ? <Icon type="antdesign" name="check" color={Colors.brandRed} size={12} /> : null}
      </ExpandPanelItem>
    );
  },
  (prevProps, nextProps) => {
...
const CategoryFilterHeader = ({
  ...
  return (
    <>
      <RowSpaceBetween
        full
        height={hp(HeaderHeight)}
        backgroundColor={collapsed ? 'transparent' : Colors.grey6}>
        <Box flexShrink={1} marginX={2} alignItems="center">
          {collapsed ? (
            ...
            <ExpandPanelTitleText>{t('category:note')}</ExpandPanelTitleText>
          )}
        </Box>
        ...
        <Button
          icon={
            collapsed ? (
              <Icon name="menu" size={20} type="ionicons" color="white" />
            ) : (
              <Icon name="chevron-thin-up" size={20} type="entypo" color="white" />
            )
          }
          iconRight={!collapsed}
      ...
      <Box position="absolute" left={0} top={hp(HeaderHeight)} zIndex={99} width="100%">
        <Collapsible collapsed={collapsed} align="top" renderChildrenCollapsed={false} style={{}}>
          <Box
            ...
            <RowCenter full hPadding>
              <ResetButton>
                <ReSetButtonText> {t('common:Reset')}</ReSetButtonText>
              </ResetButton>
              <SetButton>
                <SetButtonText> {t('common:determine')}</SetButtonText>
              </SetButton>
            </RowCenter>
          </Box>
        </Collapsible>
