...
const LikeProductsPanel = ({ products, onPressProductItem }) => {
  const navigation = useNavigation();
  const { t, i18n } = useTranslation();

  const renderItem = useCallback(
    ({ item }) => {
      return <ProductCard product={item} onPressItem={onPressProductItem} />;
    },
    [onPressProductItem],
  );
  ...
  return (
    <Box flexGrow={1}>
      <TitleText>{t('home:you may also like')}</TitleText>
...
LikeProductsPanel.propTypes = {
  products: PropTypes.arrayOf(PropTypes.object),
  onPressProductItem: PropTypes.func,
};
LikeProductsPanel.defaultProps = {
  products: [],
  onPressProductItem: () => {},
};
export default React.memo(LikeProductsPanel);
