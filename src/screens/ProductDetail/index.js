...
import { Content, Container, DismissKeyboardView } from '@components';
...
import { RowBox, RootBox } from '@src/common/StyledComponents';
...
import { SelectAttributesPanel, AddressListPanel } from './containers';
...
const ProductDetail = ({ navigation, route }) => {
  ...
  // console.log('render ProductDetail', new Date());
  const {
    deliveryAddresses,
    billingAddresses,
  ...
  const isExistProduct = useMemo(() => {
    return !isFetchingProduct && productQuantity > 0;
  }, [isFetchingProduct, productQuantity]);
  ...
  const checkExistingProduct = () => {
    if (isExistProduct) {
      ...
      Toast.show(`There is no Product `, {
  ...
  const onNavigateCheckOut = async () => {
    try {
      await callClearDeliveryRate();
      navigation.navigate('CheckoutOneTime');
    } catch (error) {}
  };
  const onCallAddProductOnCart = async () => {
    try {
      await callClearDeliveryRate();
      addProductToCart();
    } catch (error) {}
    ...
    <>
      <Container>
        ...
        <DismissKeyboardView
          ...
          <View
            style={{
              marginBottom: hp(10.28),
              paddingHorizontal: wp(28.4),
              paddingVertical: hp(12.5),
              width: '100%',
            }}>
            {/* // pls don't to use BOX as parent element, dropdown will not work */}
            <AddressListPanel
              activeAddressID={activeDeliveryAddressId}
              addresses={deliveryAddresses}
              onChangeActiveAddressId={updateActiveDeliveryAddressId}
              title={t('common:Shipping address')}
              onAddAddress={addDeliveryAddress}
              onEditAddress={updateDeliveryAddress}
              onDeleteAddress={deleteDeliveryAddress}
              activeAddress={activeDeliveryAddress}
          ...
          <View
            ...
            {/* // pls don't to use BOX as parent element, dropdown will not work */}
            <AddressListPanel
              activeAddressID={activeBillingAddressId}
              addresses={billingAddresses}
              onChangeActiveAddressId={updateActiveBillingAddressId}
              title={t('common:Billing Address')}
              onAddAddress={addBillingAddress}
              onEditAddress={updateBillingAddress}
              onDeleteAddress={deleteBillingAddress}
              activeAddress={activeBillingAddress}
